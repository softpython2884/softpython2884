> **Proposition — ce n'est pas le profil.** Le README en ligne reste `README.md`, il n'a pas bougé.
> Deuxième passe : OpenCoperLock remonte tout en haut, Capibara reprend son nom, le détail
> d'implémentation redescend d'un cran au profit de la description du projet, les estimations
> reviennent sur les cartes, la section coût est recentrée, les notes du certificat sont corrigées
> et le graphe des langages cassé est remplacé.
> Récapitulatif des changements **[en bas de page](#ce-qui-a-changé-depuis-la-v1)**.
...
**1. NationQuest** — retiré du About.

**2. Capibara s'appelle Capibara.** Plus de « Colibri — Capibara » dans le titre, plus de mention
nulle part. Le moteur reste interne, il n'a rien à faire sur un profil public.

**3. Moins de plomberie, plus de description.** J'avais sur-corrigé : à force de remplacer les
adjectifs par des mécanismes, on ne savait plus *ce que fait* le produit. Chaque carte suit
maintenant trois temps — **ce que c'est** (une phrase que ta mère comprend), **le choix technique
qui compte** (une, pas dix), **ce qu'il coûte**.

| Avant (v1) | Maintenant |
|---|---|
| ``CREATE SCHEMA t_<key>`` et ``tenant.sql`` | « chaque client a sa propre copie du schéma, donc les données d'une société ne peuvent pas ressortir dans la requête d'une autre » |
| « uid non-root, rootfs RO, `no-new-privileges`, `CAP_SYS_CHROOT` retiré » | « le bac à sable refuse les privilèges dont un vrai démon SSH a besoin » |
| « `ModuleExecutor` parcourt le graphe » | « rien n'est compilé : l'éditeur enregistre un graphe, un interpréteur le parcourt » |

Et chaque carte commence par la phrase qui manquait : Capibara *« le logiciel sur lequel une petite
entreprise passe sa journée »*, OPUS *« la plateforme sur laquelle tourne Forge Network »*, Marcus
*« un bot que les propriétaires de serveur étendent eux-mêmes, en dessinant »*, Elipse *« une app de
terrain, dans la voiture, sans réseau et sans compte »*.

**4. Les estimations reviennent sur les cartes.** Badge coût + badge mois-homme sur chacune des
cinq, comme avant. La section détaillée reste en dessous pour la méthode.

**5. La section coût est recentrée.** Elle est passée de bloc aligné à gauche à un ensemble centré
comme le reste : titre à icône, tableau centré, méthode découpée en quatre petits paragraphes
(*pas de l'argent dépensé* · *comment* · *un mois-homme c'est quoi* · *deux réserves*) au lieu d'un
pavé. Même patron visuel que la section Certified.

**6. OpenCoperLock remonte au-dessus de Flagship Work**, sous son propre titre, avec le sous-titre
*« the one you can read »* et un bouton **Read the source**. La logique de lecture devient : voilà
le code que tu peux vérifier → voilà ce que je livre et que tu ne peux pas voir. C'est plus fort
dans ce sens.

**7. Les notes du certificat sont corrigées** — Frontend et Backend étaient inversés sur GitHub,
Bases de données et SEO trop bas :

| | GitHub avant | Réel |
|---|---:|---:|
| Frontend — React / Vue / Next.js / TypeScript | 9 | **8** |
| Backend — Node.js / REST APIs / Next.js | 8 | **9** |
| DevOps — Git / GitHub / CI-CD | 10 | 10 |
| Systèmes & réseau — Linux / Windows / GCP | 10 | 10 |
| Bases de données — SQL / Prisma / PostgreSQL / MariaDB / Redis | 8 | **9** |
| SEO technique & performance web | 9 | **10** |

Les intitulés complets remplacent les libellés courts : une note est plus crédible quand on voit sur
quoi elle porte.

### Le 404 du graphe des langages

Ce n'était pas l'URL. **`github-readme-stats` est en pause côté Vercel** — le service répond
`503 DEPLOYMENT_PAUSED` pour tout le monde, c'est un projet gratuit qui se fait suspendre
régulièrement. Trois options :

1. attendre qu'il revienne (il retombera) ;
2. héberger sa propre instance (une de plus à maintenir) ;
3. **compter nous-mêmes** — c'est ce que j'ai fait.

Nouveau `tools/languages.py` : il parcourt les huit dépôts, compte les lignes par langage et sort le
tableau. Résultat, **717 526 lignes**, TypeScript 82,8 %, Dart 10,3 %.

Deux avantages qui n'ont rien à voir avec la panne : ça ne peut plus casser, et surtout **ça compte
le privé**. Le widget GitHub ne voyait que les dépôts publics, c'est-à-dire la plus petite moitié —
il affichait Java et Python en tête d'un profil où 83 % de ce qui est écrit est du TypeScript. Il se
contredisait avec le reste de la page.

### Ce qui reste supprimé de la v1

Toujours en attente de ton avis — ce sont des choix :

- **« Un nouveau Grand Projet arrive. »**
- **les badges « Team — ~46 people »** (l'info reste dans la section coût)
- **`### *From the kernel up.*`** en signature
- la ligne *« Public repos are my playground »* du bandeau animé

---

**Pour appliquer :** `cp PREVIEW.md README.md`, puis retirer le bandeau du haut et cette section.
